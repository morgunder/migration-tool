...
const shouldQuoteIdentifier = (identifier: string): boolean => {
	return (
		POSTGRES_RESERVED_WORDS.has(identifier.toLowerCase()) ||
...
const CsvToSql = () => {
		...
		reader.onload = (e) => {
			...
			try {
				...
				for (const header of headers) {
					...
					const exactMatch = schemaInfo.columns.find(
						(col) => col.name === header
					);
					if (exactMatch) {
				...
				const values = lines.slice(1).map((line) =>
					line.map((val, index) => {
						...
						console.log(
							`Header: ${header}, Value: "${val}", Type: ${type}, Result: ${result}`
						);
						return result;
					})
				);
				...
				const sql =
					`INSERT INTO ${quoteIdentifier(tableName)} (${quotedHeaders.join(
						", "
					)})\nVALUES\n` +
	...
	return (
		<div className='min-h-screen flex flex-col'>
			<header className='px-6 py-4 border-b flex-none'>
				<h1 className='text-base font-bold'>CSV TO SQL MIGRATION</h1>
			</header>
			...
			<div className='p-6 flex flex-1'>
				...
				<div className='w-1/3 pr-6'>
					<div className='space-y-6'>
						<div>
							<label className='text-med font-medium mb-2 block'>
								Table Schema
							</label>
							<Textarea
								...
								className='font-mono text-xs overflow-auto whitespace-pre'
								rows={10}
							/>
							{extractedTableName && (
				...
				{/* Right Column */}
				<Separator orientation='vertical' className='mx-6 flex-none' />
				<div className='w-2/3 pl-6 flex flex-col'>
					<div className='flex items-center gap-2 mb-4'>
						<label className='text-med font-medium'>SQL Insert Migration</label>
						{sqlOutput && (
					...
					{sqlOutput && (
						<Textarea
							value={sqlOutput}
							readOnly
							className='font-mono text-xs flex-1 overflow-auto whitespace-pre'
							rows={10}
						/>
					)}
				</div>
